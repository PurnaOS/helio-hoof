...
async function checkRecentAnalyses() {
  try {
    ...
    recentRecords.forEach((record, index) => {
      ...
      console.log(`   📅 Created: ${record.createdAt ? new Date(record.createdAt).toLocaleString() : '(unknown)'}`);
      console.log('');
    });
    ...
    const recordsWithoutDescriptions = recentRecords.filter(r => !r.description);

    if (recordsWithoutNames.length > 0) {
      console.log(`⚠️  WARNING: ${recordsWithoutNames.length} recent records have null names`);
    }

    if (recordsWithoutDescriptions.length > 0) {
      console.log(`ℹ️  INFO: ${recordsWithoutDescriptions.length} recent records have null descriptions (this is expected for some cases)`);
    }

    if (recordsWithoutNames.length === 0) {
      console.log("✅ All recent records have names - this suggests the issue may be resolved");
    }
